...
peg validate ${BASEDIR}/cluster-process-master.yml
peg validate ${BASEDIR}/cluster-process-workers.yml

read -p "Are you sure to deploy the clusters? Y/n" -n 1 -r
echo
if [[ $REPLY =~ ^[Yy]$ ]]
then
peg up ${BASEDIR}/cluster-process-master.yml &
peg up ${BASEDIR}/cluster-process-workers.yml &
