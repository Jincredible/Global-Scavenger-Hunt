...
session.execute('CREATE TABLE user_location (user_id text, time_stamp bigint, longitude double, latitude double, PRIMARY KEY (user_id, time_stamp)) WITH CLUSTERING ORDER BY (time_stamp ASC);')

session.execute('DROP TABLE IF EXISTS user_target;')
session.execute('CREATE TABLE user_target (user_id text, target_id text, time_stamp bigint, transaction_type int, PRIMARY KEY (user_id, target_id, time_stamp));')
